...
fn handle_normal_mode(key: KeyEvent, app: &mut App) -> Result<bool> {
    match key.code {
        ...
        KeyCode::Char('r') => {
            app.reload()?;
            app.set_message(app.lang.msg_reloaded());
        }
        KeyCode::Char('A') => {
